    ...
    return (
        <header className='fixed top-0 left-0 m-auto max_padd_container w-screen bg-white ring-1 ring-slate-900/5 z-10'>
            <div className='px-4 flexBetween py-3 max-xs:px-2'>
                ...
                {/* NavBar Desktop */}
                <NavBar containerStyles={"hidden lg:flex sm:justify-start gap-x-5  xl:gap-x-10 medium-15  "} />

                {/* Navbar Mobile  */}
                <NavBar containerStyles={`${menuOpened ?
                    "flex flex-col gap-y-12 fixed top-20 right-8 p-12 bg-white rounded-3xl shadow-md w-64 medium-16 ring-1 ring-slate-900/5 transition-all duration-300"
                    :
                    "flex flex-col gap-y-12 fixed top-20 p-12 bg-white rounded-3xl shadow-md w-64 medium-16 ring-1 ring-slate-900/5 transition-all duration-300 -right-[100%]"}`} />
